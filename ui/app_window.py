...
class AppWindow:
    ...
    def _on_screen_captured(self, base64_images: list):
        ...
        self._input._start_time = __import__("time").time()
    ...
    def _reinitialize_listeners(self):
        try:
            ...
            self._input.set_status("✅ Reset done — ready.")
        except Exception as ex:
            self._input.set_status(f"⚠️ Reinit error: {str(ex)}")

    # ── System Audio Callbacks ────────────────────────────────────────
    def _on_speech_recognized(self, text: str):
        self._root.after(0, lambda: self._handle_recognized(text))
    ...
    def _on_speech_recognizing(self, text: str):
        self._root.after(
            0, lambda: self._input.set_partial_text(f"🔊 {text}")
        )

    # ── Mic Callbacks ─────────────────────────────────────────────────
    def _on_mic_recognized(self, text: str):
        self._root.after(0, lambda: self._handle_mic_recognized(text))
    ...
    def _on_listener_status(self, msg: str):
        self._root.after(0, lambda: self._input.set_status(msg))
    ...
    def _on_error(self, error_msg: str):
        self._root.after(0, lambda: self._display_error(error_msg))
