...
class AudioListener:
    ...
    def _do_start(self):
        try:
            ...
            # ✅ System audio = default speaker output (loopback)
            ...
            # ✅ Fallback to default mic if loopback fails
            print(f"⚠️ Loopback failed: {ex} — trying mic fallback")
            try:
                speech_config = self._build_config()
                audio_config  = speechsdk.audio.AudioConfig(
                ...
                self._recognizer\
                    .start_continuous_recognition_async().get()
                self._is_listening = True
                self._on_status("🔊 System audio ON (mic fallback)...")
            except Exception as ex2:
                self._on_status(
                    f"⚠️ System audio error: {str(ex2)}"
                )
                self._is_listening = False
